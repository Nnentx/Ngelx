...
const ngelxPremiumPurple = Color(0xFF6F49E8);
const ngelxPremiumPurple2 = Color(0xFF8A5CFF);
const ngelxPremiumLilac = Color(0xFFF6F1FF);
const ngelxPremiumSurface = Color(0xFFFFFFFF);
const ngelxPremiumInk = Color(0xFF16121F);
const ngelxPremiumMuted = Color(0xFF746F82);
...
class NgelXPremiumCard extends StatelessWidget {
  ...
  const NgelXPremiumCard({
    super.key,
    required this.child,
    this.padding=const EdgeInsets.all(14),
    this.margin,
    this.color=ngelxPremiumSurface,
    this.radius=22,
    this.onTap,
  });
...
Widget ngelxPremiumIconButton({
  ...
  child:InkWell(
    ...
    child:Container(
      ...
      decoration:BoxDecoration(
        color:filled?const Color(0xFFEFE6FF):Colors.white.withValues(alpha:.72),
        borderRadius:BorderRadius.circular(16),
        border:Border.all(color:const Color(0xFFE8DDF8)),
      ),
      child:Icon(icon,color:color??ngelxPremiumPurple,size:23),
    ),
  ),
);
...
const ceviriler = <String, Map<String,String>>{
  ...
  'settings': {'tr':'Ayarlar','en':'Settings','de':'Einstellungen','ar':'الإعدادات','ru':'Настройки'},
  'posts': {'tr':'Gönderiler','en':'Posts','de':'Beiträge','ar':'المنشورات','ru':'Публикации'},
  'reels': {'tr':'Reels','en':'Reels','de':'Reels','ar':'ريلز','ru':'Reels'},
  'tagged': {'tr':'Etiketlenenler','en':'Tagged','de':'Markiert','ar':'الإشارات','ru':'Отмеченное'},
  'liked': {'tr':'Beğenilenler','en':'Liked','de':'Gefällt mir','ar':'الإعجابات','ru':'Понравившееся'},
  'profilePreview': {'tr':'Profil önizleme','en':'Profile preview','de':'Profilvorschau','ar':'معاينة الملف الشخصي','ru':'Предпросмотр профиля'},
  'profile': {'tr':'Profil','en':'Profile','de':'Profil','ar':'الملف الشخصي','ru':'Профиль'},
  'createAccountShort': {'tr':'Hesap Oluştur','en':'Create account','de':'Konto erstellen','ar':'إنشاء حساب','ru':'Создать аккаунт'},
...
class _AnaEkranState extends State<AnaEkran> {
            ...
            return SafeArea(
              child:Align(
                ...
                child:Container(
                  ...
                  decoration:BoxDecoration(
                    gradient:const LinearGradient(colors:[Color(0xFF211837),Color(0xFF2D1D48)],begin:Alignment.topLeft,end:Alignment.bottomRight),
                    borderRadius:BorderRadius.circular(24),
                    border:Border.all(color:Colors.white10),
                    boxShadow:const [BoxShadow(color:Color(0x55000000),blurRadius:28,offset:Offset(0,10))],
                  ),
                  child:Row(children:[
                    Stack(clipBehavior:Clip.none,children:[
                      Container(
                        padding:const EdgeInsets.all(2),
                        decoration:const BoxDecoration(shape:BoxShape.circle,gradient:LinearGradient(colors:[Color(0xFFB47BFF),Color(0xFF7048E8)])),
                        child:CircleAvatar(
                          radius:25,
                      ...
                      const Positioned(right:-1,bottom:0,child:CircleAvatar(radius:7,backgroundColor:Color(0xFF211837),child:CircleAvatar(radius:4,backgroundColor:Color(0xFF22C66D)))),
                    ]),
                    const SizedBox(width:11),
                    Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisSize:MainAxisSize.min,children:[
                      Text(baslik,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(color:Colors.white,fontWeight:FontWeight.w900,fontSize:15.5)),
                      const SizedBox(height:3),
                      Row(children:[
                        Icon(goruntulu?Icons.videocam_rounded:Icons.call_rounded,color:const Color(0xFFCDB9FF),size:15),
                        const SizedBox(width:5),
                        Text(goruntulu?'Gelen görüntülü arama':'Gelen sesli arama',style:const TextStyle(color:Color(0xFFC8BDD4),fontSize:10.8,fontWeight:FontWeight.w700)),
                      ]),
                    ])),
                    _gelenAramaButon(
                      tooltip:'Reddet',
                    ...
                    _gelenAramaButon(
                      tooltip:'Cevapla',
                      icon:goruntulu?Icons.videocam_rounded:Icons.call_rounded,
                      color:const Color(0xFF29C76F),
                      onPressed:()async{
  ...
  Widget _gelenAramaButon({required String tooltip,required IconData icon,required Color color,required Future<void> Function() onPressed})=>Tooltip(
    message:tooltip,
    child:InkWell(
      onTap:()=>onPressed(),
      borderRadius:BorderRadius.circular(18),
      child:Container(
        width:43,height:43,
        decoration:BoxDecoration(color:color,shape:BoxShape.circle,boxShadow:[BoxShadow(color:color.withValues(alpha:.28),blurRadius:12,offset:const Offset(0,5))]),
...
class _KesfetPageState extends State<KesfetPage> {
  ...
  void _aramaAc(){
    Navigator.push(context,MaterialPageRoute(builder:(_)=>const AramaPage()));
  }

  Future<void> _filtreAc()async{
        ...
        builder:(_,snap){
          ...
          return SliverGrid(
            delegate:SliverChildBuilderDelegate((_,i){
              ...
              return GestureDetector(
                onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>IcerikBaglantiPage(icerikId:belgeler[i].id))),
  ...
  List<Widget> _kisiSliverleri()=>[
    SliverToBoxAdapter(child:Padding(padding:const EdgeInsets.fromLTRB(16,20,16,10),child:Text(t('discoverPeople'),style:const TextStyle(color:Colors.black,fontSize:21,fontWeight:FontWeight.w900)))),
    StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
  ...
  Widget _kesfetSekmesi(IconData ikon,String yazi,int index)=>Padding(
    padding:const EdgeInsets.only(right:10),
    child:InkWell(
      onTap:()=>setState(()=>kategori=index),
      borderRadius:BorderRadius.circular(22),
      child:AnimatedContainer(
        duration:const Duration(milliseconds:180),
        padding:const EdgeInsets.symmetric(horizontal:20),
        decoration:BoxDecoration(
          gradient:kategori==index?const LinearGradient(colors:[Color(0xFF7C3AED),Color(0xFFA855F7)]):null,
          color:kategori==index?null:const Color(0xFFF2F3F6),
          borderRadius:BorderRadius.circular(22),
        ),
        child:Row(children:[Icon(ikon,size:18,color:kategori==index?Colors.white:Colors.black54),const SizedBox(width:7),Text(yazi,style:TextStyle(color:kategori==index?Colors.white:Colors.black87,fontWeight:FontWeight.w700))]),
      ),
    ),
  );

  Widget _kisiSatiri(String uid,Map<String,dynamic> v){
    final foto=(v['photoUrl']??'').toString();
  ...
  Widget _bosKart(String yazi,IconData ikon)=>Container(
    margin:const EdgeInsets.symmetric(horizontal:16),
    decoration:BoxDecoration(color:const Color(0xFFF7F7FA),borderRadius:BorderRadius.circular(20),border:Border.all(color:Colors.black.withValues(alpha:.06))),
    child:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[Icon(ikon,color:Colors.black38,size:40),const SizedBox(height:8),Padding(padding:const EdgeInsets.symmetric(horizontal:18),child:Text(yazi,textAlign:TextAlign.center,style:const TextStyle(color:Colors.black54,fontWeight:FontWeight.w600)))])),
  );

  void _etiketAra(String etiket){
    Navigator.push(context,MaterialPageRoute(builder:(_)=>AramaPage(baslangicSorgu:etiket)));
  }
}
...
class _YeniYuklePageState extends State<YuklePage> {
  ...
  Future<void> hikayeSecimi()async{
    if(yukleniyor)return;
    await showModalBottomSheet<void>(
      context:context,
      backgroundColor:Colors.white,
      ...
      builder:(c)=>Theme(data:ThemeData.light(),child:SafeArea(child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        Row(children:[
          ...
          Expanded(child:ListTile(
            leading:const Icon(Icons.video_library_outlined,color:mor),
            title:const Text('Galeriden video'),
            onTap:(){Navigator.pop(c);unawaited(_hikayePaylas(ImageSource.gallery,video:true));},
          )),
        ]),
        Row(children:[
          Expanded(child:ListTile(
  ...
  Future<void> yayinla()async{
    ...
    setState((){
      yukleniyor=true;
      yuklemeIlerlemesi=0;
      yuklemeDurumu=tur=='text'?'Gönderi yayınlanıyor...':'Medya yükleniyor...';
    });
    ...
    try{
      ...
      final adi=(pv['username']??'ngelx').toString();
      await FirebaseFirestore.instance.collection('videos').add({
        ...
        'privacy':gizlilik,
        'visibleTo':gizlilik=='Yakın arkadaşlar'?List<String>.from(pv['closeFriends']??const[]):<String>[],
        'hiddenFor':<String>[],
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        ...
        action:SnackBarAction(
          label:'Akışa git',
          onPressed:(){
            final ana=context.findAncestorStateOfType<_AnaEkranState>();
            if(ana!=null&&ana.mounted)ana.setState(()=>ana.secili=0);
          },
        ),
      ));
    }catch(e){
        ...
        child:SafeArea(child:ListView(
          ...
          children:[
            ...
            if(yukleniyor)...[
              ...
              const SizedBox(height:10),
              Text(
                yuklemeDurumu.isEmpty?'İşlem devam ediyor...':yuklemeDurumu,
                textAlign:TextAlign.center,
  ...
  Widget _turButonu(String deger,IconData ikon,String yazi){
    ...
    return Expanded(child:GestureDetector(
      onTap:yukleniyor?null:()=>_turDegistir(deger),
      child:AnimatedContainer(
        duration:const Duration(milliseconds:220),
        padding:const EdgeInsets.symmetric(vertical:14),
        decoration:BoxDecoration(
          gradient:secili?const LinearGradient(colors:[mavi,mor]):null,
          color:secili?null:const Color(0xFFF3F5F8),
          borderRadius:BorderRadius.circular(17),
        ),
        child:Column(children:[
          Icon(ikon,color:secili?Colors.black:Colors.black54),
          const SizedBox(height:5),
          Text(yazi,style:TextStyle(color:secili?Colors.black:Colors.black54,fontWeight:FontWeight.bold)),
        ]),
      ),
    ));
  }
...
class _GrubaKatilPageState extends State<GrubaKatilPage>{
  final kod=TextEditingController();
  bool yukleniyor=false;

  @override void dispose(){kod.dispose();super.dispose();}

  String _koduAyikla(String ham){
    var t=ham.trim();
    if(t.isEmpty)return '';
    final uri=Uri.tryParse(t);
    if(uri!=null&&uri.pathSegments.isNotEmpty&&uri.pathSegments.contains('join')){
      final i=uri.pathSegments.indexOf('join');
      if(i>=0&&i+1<uri.pathSegments.length)return uri.pathSegments[i+1].trim();
    }
    if(t.contains('/join/'))t=t.split('/join/').last;
    if(t.contains('?'))t=t.split('?').first;
    return t.trim();
  }

  Future<void> katil()async{
    final me=FirebaseAuth.instance.currentUser?.uid;
    if(me==null||yukleniyor)return;
    final invite=_koduAyikla(kod.text);
    if(invite.isEmpty){
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Davet bağlantısı veya kodunu gir.')));
      return;
    }
    setState(()=>yukleniyor=true);
    try{
      ...
      body:ListView(
        ...
        children:[
          NgelXPremiumCard(
            ...
            child:Column(children:[
              Container(width:78,height:78,decoration:BoxDecoration(color:const Color(0xFFF0E8FF),borderRadius:BorderRadius.circular(26)),child:const Icon(Icons.group_add_rounded,color:ngelxPremiumPurple,size:40)),
              const SizedBox(height:15),
              const Text('Davetle gruba katıl',textAlign:TextAlign.center,style:TextStyle(color:ngelxPremiumInk,fontSize:21,fontWeight:FontWeight.w900)),
              const SizedBox(height:6),
              const Text('Sana gönderilen NgelX grup bağlantısını veya davet kodunu aşağıya yapıştır.',textAlign:TextAlign.center,style:TextStyle(color:ngelxPremiumMuted,fontSize:12,height:1.4,fontWeight:FontWeight.w600)),
            ]),
          ),
          const SizedBox(height:14),
          NgelXPremiumCard(
            padding:const EdgeInsets.all(14),
            child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
              const Text('Davet bağlantısı / kodu',style:TextStyle(color:ngelxPremiumInk,fontWeight:FontWeight.w900)),
              const SizedBox(height:10),
              TextField(
                controller:kod,
                ...
                onSubmitted:(_)=>katil(),
                decoration:InputDecoration(
                  hintText:'ngelxsocial.com/join/...',
                  prefixIcon:const Icon(Icons.link_rounded,color:ngelxPremiumPurple),
                  suffixIcon:IconButton(
                    tooltip:'Yapıştır',
                    onPressed:()async{
                      final d=await Clipboard.getData(Clipboard.kTextPlain);
                      if(d?.text!=null)kod.text=d!.text!;
                    },
                    icon:const Icon(Icons.content_paste_rounded,color:ngelxPremiumPurple),
                  ),
              ...
              SizedBox(
                ...
                child:FilledButton.icon(
                  ...
                  onPressed:yukleniyor?null:katil,
                  icon:yukleniyor?const SizedBox(width:18,height:18,child:CircularProgressIndicator(strokeWidth:2,color:Colors.white)):const Icon(Icons.login_rounded),
                  label:Text(yukleniyor?'Kontrol ediliyor...':'Gruba katıl',style:const TextStyle(fontWeight:FontWeight.w800)),
                ),
              ),
            ]),
          ),
          ...
          const NgelXPremiumCard(
            ...
            child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[
              ...
              SizedBox(width:10),
              Expanded(child:Text('Grup yöneticisi onay istiyorsa önce katılma isteğin gönderilir. Onaylandığında bildirim alırsın.',style:TextStyle(color:ngelxPremiumMuted,fontSize:11.5,height:1.4,fontWeight:FontWeight.w600))),
            ]),
          ),
        ],
      ),
...
class _GrupSohbetPageState extends State<GrupSohbetPage>{
  ...
  Future<void> grupSesKaydiDegistir()async{
    ...
    try{
      if(!await _grupSesKaydedici.hasPermission()){
        if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Mikrofon izni gerekli.')));
        return;
      }
      final dir=await getTemporaryDirectory();
      final yol=dir.path+'/ngelx_group_voice_'+DateTime.now().millisecondsSinceEpoch.toString()+'.m4a';
      await _grupSesKaydedici.start(
  ...
  Future<void> _typingTemizle()async{
    final me=uid;
    if(me==null)return;
    _typingYazildi=false;
    try{await chatRef.set({'typing_'+me:FieldValue.delete()},SetOptions(merge:true));}catch(_){}
  }

  void _typingGuncelle(String deger){
    final me=uid;
    if(me==null)return;
    _typingZamanlayici?.cancel();
          ...
          child:ConstrainedBox(
            ...
            NgelXPremiumCard(
              ...
              child:Row(mainAxisAlignment:MainAxisAlignment.spaceAround,children:[
                for(final e in const ['❤️','👍','😂','😮','😢','😡'])
                  InkWell(
                    onTap:()=>Navigator.pop(c,'reaction:$e'),
                    borderRadius:BorderRadius.circular(16),
            ...
            NgelXPremiumCard(
              ...
              child:Column(children:[
                _mesajMenuSatir(c,Icons.reply_rounded,'Yanıtla','Mesaja yanıt ver','reply'),
                if(metin.isNotEmpty)...[
                  const Divider(height:1,indent:62,color:Color(0xFFF0EBF5)),
                  _mesajMenuSatir(c,Icons.copy_rounded,'Kopyala','Metni panoya kopyala','copy'),
                ],
                if(ben&&metin.isNotEmpty)...[
                  const Divider(height:1,indent:62,color:Color(0xFFF0EBF5)),
                ...
                if(!ben)...[
                  const Divider(height:1,indent:62,color:Color(0xFFF0EBF5)),
                  _mesajMenuSatir(c,Icons.flag_outlined,'Şikâyet et','Bu mesajı bildir','report'),
                ],
              ]),
            ),
            if(ben||yonetici)...[
              ...
              InkWell(
                ...
                child:Container(
                  ...
                  child:const Row(children:[
                    ...
                    SizedBox(width:12),
                    Text('Herkesten sil',style:TextStyle(color:Color(0xFFE13F51),fontWeight:FontWeight.w900)),
                  ]),
                ),
              ),
            ],
    ...
    if(sec.startsWith('reaction:')){
      ...
      final ok=await showDialog<bool>(
        ...
        builder:(c)=>AlertDialog(
          shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(26)),
          backgroundColor:Colors.white,surfaceTintColor:Colors.white,
          icon:const Icon(Icons.delete_outline_rounded,color:Color(0xFFE13F51),size:34),
          title:const Text('Mesaj silinsin mi?',style:TextStyle(color:ngelxPremiumInk,fontWeight:FontWeight.w900)),
          content:const Text('Mesaj gruptan kalıcı olarak kaldırılacak.',textAlign:TextAlign.center,style:TextStyle(color:ngelxPremiumMuted)),
          actions:[
            TextButton(onPressed:()=>Navigator.pop(c,false),child:const Text('Vazgeç')),
            FilledButton(style:FilledButton.styleFrom(backgroundColor:const Color(0xFFE13F51)),onPressed:()=>Navigator.pop(c,true),child:const Text('Sil')),
          ],
        ),
      )??false;
      if(ok){
      ...
      final ok=await showDialog<bool>(
        ...
        builder:(x)=>AlertDialog(
          ...
          actions:[
            ...
            FilledButton(onPressed:()=>Navigator.pop(x,true),child:const Text('Kaydet')),
          ],
        ),
      )??false;
      if(ok&&c.text.trim().isNotEmpty)await d.reference.update({'text':c.text.trim(),'editedAt':FieldValue.serverTimestamp()});
      c.dispose();
    }
  }
      ...
      builder:(c)=>Theme(
        ...
        child:SafeArea(child:SingleChildScrollView(child:Padding(
          ...
          child:Column(mainAxisSize:MainAxisSize.min,crossAxisAlignment:CrossAxisAlignment.start,children:[
            ...
            const SizedBox(height:4),
            const Text('Gruba bir şey paylaş.',style:TextStyle(color:ngelxPremiumMuted,fontSize:11.5,fontWeight:FontWeight.w600)),
            const SizedBox(height:16),
            Row(children:[
              Expanded(child:_ekSecenek(c,Icons.camera_alt_rounded,'Kamera','camera')),
              const SizedBox(width:10),
              Expanded(child:_ekSecenek(c,Icons.photo_library_rounded,'Galeri','gallery')),
              const SizedBox(width:10),
            ...
            child:Column(children:[
              ...
              StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
                ...
                builder:(_,pinSnap){
                  ...
                  final v=pins.first.data(),tur=(v['type']??'text').toString();
                  final metin=(v['text']??'').toString().trim();
                  final ozet=metin.isNotEmpty?metin:(tur=='photo'?'Fotoğraf':tur=='gif'?'GIF':tur=='audio'?'Sesli mesaj':'Sabitlenen mesaj');
                  return InkWell(
                    ...
                    child:Container(
                      ...
                      child:Row(children:[
                        ...
                        const Icon(Icons.chevron_right_rounded,color:Color(0xFFA39AAB),size:20),
                      ]),
                    ),
                  );
                },
          ...
          builder:(_,snap){
            ...
            if(sekme==0){
              final imageUrls=<String>[
                for(final d in docs)
                  if((d.data()['type']??'').toString()!='video'&&(d.data()['mediaUrl']??'').toString().isNotEmpty)
                    (d.data()['mediaUrl']??'').toString(),
              ];
                ...
                itemBuilder:(_,i){
                  ...
                  return InkWell(
                    onTap:url.isEmpty?null:()=>Navigator.push(context,MaterialPageRoute(
                      builder:(_)=>t=='video'
                        ?TamEkranVideoPage(url:url)
                        :NgelXMedyaGaleriPage(urls:imageUrls,initialIndex:imageIndex<0?0:imageIndex),
                    )),
                    borderRadius:BorderRadius.circular(16),
                    child:ClipRRect(
                      borderRadius:BorderRadius.circular(16),
              ...
              return ListView.separated(
                ...
                itemBuilder:(_,i){
                  ...
                  return NgelXPremiumCard(
                    ...
                    child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                      Text('Ses kaydı • '+mesajSaati(data['createdAt']),style:const TextStyle(color:ngelxPremiumMuted,fontSize:11,fontWeight:FontWeight.w700)),
                      const SizedBox(height:8),
                      NgelXSesliMesaj(url:url,benim:false,durationSeconds:(data['durationSeconds'] as num?)?.toInt()??0),
                    ]),
                  );
                },
              );
  ...
  Widget _sekme(String label,int index)=>Expanded(
    child:InkWell(
      ...
      child:AnimatedContainer(
        ...
        height:40,
        alignment:Alignment.center,
        decoration:BoxDecoration(color:sekme==index?ngelxGroupGreen:Colors.transparent,borderRadius:BorderRadius.circular(14)),
        child:FittedBox(
          fit:BoxFit.scaleDown,
          child:Text(label,style:TextStyle(color:sekme==index?Colors.white:ngelxPremiumMuted,fontSize:11.2,fontWeight:FontWeight.w800)),
        ),
      ),
    ),
  );
...
class NgelXMedyaGaleriPage extends StatefulWidget{
  final List<String> urls;
  final int initialIndex;
  const NgelXMedyaGaleriPage({super.key,required this.urls,required this.initialIndex});
  @override State<NgelXMedyaGaleriPage> createState()=>_NgelXMedyaGaleriPageState();
}
class _NgelXMedyaGaleriPageState extends State<NgelXMedyaGaleriPage>{
  late final PageController controller;
  late int index;
  @override void initState(){
    super.initState();
    index=widget.initialIndex.clamp(0,widget.urls.isEmpty?0:widget.urls.length-1);
  ...
  @override Widget build(BuildContext context)=>Scaffold(
    ...
    appBar:AppBar(
      ...
      foregroundColor:Colors.white,
      elevation:0,
      title:Text(widget.urls.isEmpty?'Medya':(index+1).toString()+' / '+widget.urls.length.toString(),style:const TextStyle(fontWeight:FontWeight.w800)),
    ),
    body:widget.urls.isEmpty
      ?const Center(child:Icon(Icons.broken_image_outlined,color:Colors.white54,size:58))
      :PageView.builder(
        ...
        itemBuilder:(_,i)=>InteractiveViewer(
          ...
          child:Center(child:CachedNetworkImage(
            ...
            errorWidget:(_,__,___)=>const Icon(Icons.broken_image_outlined,color:Colors.white54,size:58),
          )),
        ),
      ),
  );
...
class _NgelXAramaPageState extends State<NgelXAramaPage>{
  ...
  Future<DocumentSnapshot<Map<String,dynamic>>> _aramaProfil(String id)=>_aramaProfilCache.putIfAbsent(id,()=>FirebaseFirestore.instance.collection('users').doc(id).get());

  Widget _katilimciAvatar(lk.Participant p,{bool yerel=false,double radius=40}){
  ...
  Widget _katilimciKutusu(lk.Participant p,{bool yerel=false}){
    final ad=_katilimciAdi(p,yerel:yerel),track=_katilimciVideosu(p);
    return Container(
      clipBehavior:Clip.antiAlias,
      decoration:BoxDecoration(
        color:ngelxCallCard,
      ...
      child:Stack(fit:StackFit.expand,children:[
        ...
        Positioned(
          ...
          child:Container(
            ...
            child:Row(children:[
              Expanded(child:Text(ad,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(color:Colors.white,fontSize:12.5,fontWeight:FontWeight.w800))),
              Icon(yerel&&mikrofon?Icons.graphic_eq_rounded:Icons.mic_off_rounded,color:yerel&&mikrofon?const Color(0xFFB88BFF):Colors.white60,size:18),
            ]),
          ),
        ),
      ]),
    ...
    return SizedBox(
      ...
      child:Column(mainAxisSize:MainAxisSize.min,children:[
        Container(
          ...
          decoration:BoxDecoration(
            shape:BoxShape.circle,
            gradient:aktif?const LinearGradient(colors:[Color(0xFFB479FF),Color(0xFF6F49E8)]):null,
            color:aktif?null:Colors.white12,
            boxShadow:aktif?const [BoxShadow(color:Color(0x557A50E8),blurRadius:22,spreadRadius:3)]:null,
          ),
          child:Stack(clipBehavior:Clip.none,children:[
            _katilimciAvatar(p,yerel:yerel,radius:43),
            Positioned(right:-2,bottom:2,child:CircleAvatar(radius:13,backgroundColor:aktif?ngelxPremiumPurple:const Color(0xFF272036),child:Icon(aktif?Icons.graphic_eq_rounded:Icons.mic_off_rounded,color:Colors.white,size:14))),
          ]),
        ),
        const SizedBox(height:9),
  ...
  Widget _grupAramaAlani(){
    ...
    if(r==null){
      return Expanded(child:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[
        const CircularProgressIndicator(color:Color(0xFFB88BFF)),
        const SizedBox(height:14),
        Text(baglaniyor?'Grup aramasına bağlanılıyor…':'Katılımcılar bekleniyor…',style:const TextStyle(color:Colors.white70,fontWeight:FontWeight.w600)),
      ])));
    }
    final yerel=r.localParticipant;
    final katilimcilar=<({lk.Participant p,bool yerel})>[
    ...
    if(!widget.goruntulu){
      return Expanded(child:Column(children:[
        ...
        Expanded(child:Center(child:SingleChildScrollView(
          ...
          child:Wrap(alignment:WrapAlignment.center,spacing:18,runSpacing:24,children:katilimcilar.map((x)=>_grupSesliKatilimci(x.p,yerel:x.yerel)).toList()),
        ))),
      ]));
    }
    ...
    return Expanded(child:Column(children:[
      ...
      Expanded(child:GridView.builder(
        ...
        itemCount:katilimcilar.length,
        gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:sutun,crossAxisSpacing:10,mainAxisSpacing:10,childAspectRatio:katilimcilar.length<=2?1.02:.82),
  ...
  Widget _videoAlani(){
    ...
    return Expanded(child:Stack(children:[
      ...
      if(uzak!=null&&yerel!=null)Positioned(right:22,top:22,width:112,height:158,child:ClipRRect(borderRadius:BorderRadius.circular(20),child:_efektliVideo(yerel))),
      Positioned(left:30,right:30,bottom:20,child:Container(
        ...
        child:SafeArea(child:Column(children:[
          ...
          if(bitisDurumu==null&&hata==null)Padding(
            ...
            child:InkWell(
              ...
              child:Container(width:138,height:64,decoration:BoxDecoration(color:const Color(0xFFFF443F),borderRadius:BorderRadius.circular(30),boxShadow:const [BoxShadow(color:Color(0x44FF443F),blurRadius:20,offset:Offset(0,8))]),child:const Icon(Icons.call_end_rounded,color:Colors.white,size:32)),
            ),
          ),
          const SizedBox(height:22),
        ])),
  ...
  Widget ngelxCallMiniButton(IconData icon,VoidCallback onTap)=>InkWell(
    onTap:onTap,
    borderRadius:BorderRadius.circular(18),
    child:Container(width:42,height:42,decoration:BoxDecoration(color:Colors.black38,borderRadius:BorderRadius.circular(18),border:Border.all(color:Colors.white10)),child:Icon(icon,color:Colors.white,size:21)),
  );

  Widget _efektChip(String label,bool value,ValueChanged<bool> onChanged)=>FilterChip(
    label:Text(label),
    selected:value,
    onSelected:onChanged,
    selectedColor:const Color(0xFFB88BFF),
    backgroundColor:Colors.white10,
    side:BorderSide(color:Colors.white.withValues(alpha:.10)),
    labelStyle:TextStyle(color:value?ngelxCallBg:Colors.white,fontWeight:FontWeight.w800),
  );

  Widget _aramaKontrol(IconData icon,String label,Future<void> Function() onTap,bool acik)=>Column(mainAxisSize:MainAxisSize.min,children:[
    InkWell(
      onTap:()=>onTap(),
      borderRadius:BorderRadius.circular(28),
      child:Container(
        width:58,height:58,
        decoration:BoxDecoration(color:acik?Colors.white12:Colors.white,shape:BoxShape.circle,border:Border.all(color:Colors.white.withValues(alpha:.10))),
        child:Icon(icon,color:acik?Colors.white:ngelxCallBg,size:27),
      ),
    ),
    const SizedBox(height:7),
...
class _GrupBilgiPageState extends State<GrupBilgiPage>{
      ...
      builder:(x)=>Theme(
        ...
        child:AlertDialog(
          ...
          content:TextField(
            ...
            style:const TextStyle(color:ngelxPremiumInk,fontWeight:FontWeight.w600),
            decoration:InputDecoration(
              hintText:'Grubun amacını kısaca anlat...',
              filled:true,fillColor:const Color(0xFFF7F4F9),
              border:OutlineInputBorder(borderRadius:BorderRadius.circular(18),borderSide:BorderSide.none),
              enabledBorder:OutlineInputBorder(borderRadius:BorderRadius.circular(18),borderSide:const BorderSide(color:ngelxPremiumBorder)),
              focusedBorder:OutlineInputBorder(borderRadius:BorderRadius.circular(18),borderSide:const BorderSide(color:ngelxGroupGreen,width:1.5)),
            ...
            children:[
              ...
              InkWell(
                ...
                child:Container(
                  ...
                  decoration:BoxDecoration(color:const Color(0xFFFFEFF1),borderRadius:BorderRadius.circular(20),border:Border.all(color:const Color(0xFFFFD7DC))),
                  child:Row(children:[
  ...
  Widget _grupKisayol(IconData icon,String label,VoidCallback? onTap)=>InkWell(
    onTap:onTap,
    borderRadius:BorderRadius.circular(22),
    child:SizedBox(
      height:84,
      child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[
        Container(
          width:52,height:52,
          decoration:const BoxDecoration(color:ngelxGroupGreenSoft,shape:BoxShape.circle),
          child:Icon(icon,color:ngelxGroupGreen,size:25),
        ),
        const SizedBox(height:7),
        Text(label,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(color:ngelxPremiumInk,fontSize:11.5,fontWeight:FontWeight.w700)),
      ]),
    ),
  );

  Widget _grupAyirici()=>const Divider(height:1,indent:58,endIndent:8,color:Color(0xFFF0EBF6));
        ...
        builder:(_,snap){
          ...
          return ListView(
            ...
            children:[
              ...
              _satir(Icons.bubble_chart_rounded,'Sohbet balonu aç',subtitle:bubble?'Açık':'Kapalı',onTap:()=>_bool('chatBubble',!bubble)),
              _satir(Icons.auto_awesome_motion_rounded,'Anı ayarları',subtitle:memories?'Açık':'Kapalı',onTap:()=>_bool('memoriesEnabled',!memories)),
              const SizedBox(height:22),
              const Text('Gizlilik ve destek',style:TextStyle(color:Colors.black54,fontSize:17,fontWeight:FontWeight.w800)),
              const SizedBox(height:8),
              _satir(Icons.visibility_outlined,'Okundu bilgisi',subtitle:read?'Açık':'Kapalı',onTap:()=>_bool('readReceipts',!read)),
              _satir(Icons.more_horiz_rounded,'Yazma göstergesi',subtitle:typing?'Açık':'Kapalı',onTap:()=>_bool('typingIndicator',!typing)),
              ...
              _satir(Icons.delete_outline_rounded,'Sohbeti sil',onTap:()=>_sil(context)),
            ],
          );
        },
      ),
          ...
          return ListView(
            ...
            children: [
              ...
              if (!erisimVar)
                ...
                Column(children:[Padding(padding:const EdgeInsets.only(bottom:12),child:Row(mainAxisAlignment:MainAxisAlignment.spaceAround,children:[_ProfilSekme(t('posts'),true),_ProfilSekme(t('reels'),false),_ProfilSekme(t('tagged'),false)])),StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
...
class _HikayeGosterPageState extends State<HikayeGosterPage> with SingleTickerProviderStateMixin {
  ...
  bool gonderiliyor=false,sessiz=false,videoHazir=false,videoHata=false;

  bool get videoMu{
    if(widget.mediaType=='video')return true;
    final temiz=widget.url.split('?').first.toLowerCase();
  ...
  Future<void> _videoHazirla()async{
    try{
      ...
      await x.initialize();
      if(!mounted)return;
      final d=x.value.duration;
      if(d.inMilliseconds>0){
        final ms=d.inMilliseconds.clamp(1000,30000);
  ...
  void _duraklat(){
    sure.stop();
    if(videoMu&&videoKontrol?.value.isInitialized==true)unawaited(videoKontrol!.pause());
  }

  void _devam(){
    if(!mounted)return;
    sure.forward();
    if(videoMu&&videoKontrol?.value.isInitialized==true)unawaited(videoKontrol!.play());
  }

  Future<void> _sesDegistir()async{
    final yeni=!sessiz;
    setState(()=>sessiz=yeni);
    if(videoKontrol?.value.isInitialized==true)await videoKontrol!.setVolume(yeni?0:1);
  }

  @override void dispose(){
    cevap.dispose();
    sure.dispose();
    videoKontrol?.dispose();
    super.dispose();
  }
  ...
  Future<void> _yanitGonder(String ham,{bool tepki=false})async{
    ...
    try{
      ...
      batch.set(mesajRef,{
        ...
        'storyUrl':widget.url,
        'storyOwnerId':hedef,
        'storyMediaType':widget.mediaType,
        'reaction':tepki,
        'createdAt':FieldValue.serverTimestamp(),
        'clientCreatedAt':Timestamp.now(),
      });
          ...
          SafeArea(child:Padding(
            ...
            child:Column(children:[
              ...
              Row(children:[
                ...
                if(videoMu)IconButton(
                  tooltip:sessiz?'Sesi aç':'Sesi kapat',
                  onPressed:_sesDegistir,
                  icon:Icon(sessiz?Icons.volume_off_rounded:Icons.volume_up_rounded,color:Colors.white,size:29),
                ),
                IconButton(tooltip:'Hikâye seçenekleri',onPressed:(){
                  _duraklat();
...
class _HesapDegistirPageState extends State<HesapDegistirPage> {
  ...
  Future<void> _yukle()async{
    ...
    final aktifEmail=aktif?.email?.trim()??'';
    if(aktifEmail.isNotEmpty){
      tek.removeWhere((x)=>x.toLowerCase()==aktifEmail.toLowerCase());
      tek.insert(0,aktifEmail);
      await h.setString(uidAnahtari(aktifEmail),aktif!.uid);
    }
    if(tek.length>5)tek.removeRange(5,tek.length);
    await h.setStringList('hatirlanan_epostalar',tek);
    if(aktifEmail.isNotEmpty)await h.setString('hatirlanan_eposta',aktifEmail);
  ...
  Future<void> _kaldir(String email)async{
    ...
    if(aktif==email.toLowerCase()){
      if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Açık olan hesabı cihazdan kaldıramazsın. Önce başka hesaba geç.')));
      return;
    }
...
class NgelXPremiumPage extends StatelessWidget{
  ...
  void _magazaHazirDegil(BuildContext context,String urun){
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(urun+' satın alımı Google Play ödeme entegrasyonu bağlandığında aktif olacak.')));
  }
  ...
  @override Widget build(BuildContext context){
    ...
    return Theme(data:ThemeData.light(),child:Scaffold(
      ...
      appBar:AppBar(
        backgroundColor:Colors.white,surfaceTintColor:Colors.white,elevation:0,
        title:const Text('NgelX Premium',style:TextStyle(fontWeight:FontWeight.w900)),
        actions:[
          IconButton(
            tooltip:'Hesap değiştir',
            onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const HesapDegistirPage())),
      ...
      body:uid==null?const Center(child:Text('Oturum bulunamadı.')):StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(
        ...
        builder:(_,s){
          ...
          final premium=v['premiumActive']==true;
          final verified=v['verified']==true;
          final coin=(v['coinBalance'] as num?)?.toInt()??0;
          return ListView(padding:const EdgeInsets.fromLTRB(16,8,16,32),children:[
            Container(
              padding:const EdgeInsets.all(20),
              decoration:BoxDecoration(
                borderRadius:BorderRadius.circular(28),
                gradient:const LinearGradient(colors:[Color(0xFF062B8F),Color(0xFF174BD8),Color(0xFF7539ED)],begin:Alignment.topLeft,end:Alignment.bottomRight),
                boxShadow:const [BoxShadow(color:Color(0x332E4DCE),blurRadius:28,offset:Offset(0,12))],
              ),
              child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                Row(children:[
                  Container(width:44,height:44,decoration:const BoxDecoration(shape:BoxShape.circle,gradient:LinearGradient(colors:[Color(0xFF22D3EE),Color(0xFF8B5CF6)])),child:const Icon(Icons.workspace_premium_rounded,color:Colors.white)),
                  const SizedBox(width:10),
                  const Text('NgelX',style:TextStyle(color:Colors.white,fontSize:19,fontWeight:FontWeight.w900)),
                  const SizedBox(width:7),
                  Container(padding:const EdgeInsets.symmetric(horizontal:9,vertical:4),decoration:BoxDecoration(color:Colors.white.withValues(alpha:.16),borderRadius:BorderRadius.circular(12)),child:Text(premium?'PREMIUM AKTİF':'PREMIUM',style:const TextStyle(color:Colors.white,fontSize:9,fontWeight:FontWeight.w900))),
                  const Spacer(),
                  Container(padding:const EdgeInsets.symmetric(horizontal:9,vertical:6),decoration:BoxDecoration(color:Colors.black.withValues(alpha:.17),borderRadius:BorderRadius.circular(15),border:Border.all(color:Colors.white24)),child:Row(children:[const Icon(Icons.monetization_on_rounded,color:Color(0xFFFFC928),size:18),const SizedBox(width:4),Text(coin.toString(),style:const TextStyle(color:Colors.white,fontWeight:FontWeight.w900))])),
                ]),
                const SizedBox(height:22),
                Text(premium?'Premium ayrıcalıkların açık':'Premium ile öne çık',style:const TextStyle(color:Colors.white,fontSize:27,fontWeight:FontWeight.w900)),
                const SizedBox(height:6),
                Text('Özel ayrıcalıklar, jetonlar ve daha fazla görünürlük.',style:TextStyle(color:Colors.white.withValues(alpha:.80),fontSize:14,height:1.35)),
                const SizedBox(height:18),
                FilledButton.icon(
                  style:FilledButton.styleFrom(backgroundColor:Colors.white,foregroundColor:ngelxPremiumPurple,padding:const EdgeInsets.symmetric(horizontal:18,vertical:13),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(17))),
                  onPressed:premium?null:()=>_magazaHazirDegil(context,'NgelX Premium'),
                  icon:const Icon(Icons.workspace_premium_rounded),
                  label:Text(premium?'Premium aktif':'Premium’a Geç',style:const TextStyle(fontWeight:FontWeight.w900)),
                ),
              ]),
            ),
            const SizedBox(height:22),
            const Text('Premium Ayrıcalıkları',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900)),
            const SizedBox(height:10),
            GridView.count(
              shrinkWrap:true,physics:const NeverScrollableScrollPhysics(),crossAxisCount:2,crossAxisSpacing:10,mainAxisSpacing:10,childAspectRatio:1.75,
              children:const [
                _PremiumOzellik(icon:Icons.block_rounded,title:'Reklamsız kullanım',subtitle:'Kesintisiz deneyim',color:Color(0xFFFFEEF1)),
                _PremiumOzellik(icon:Icons.palette_outlined,title:'Profil temaları',subtitle:'Tarzını yansıt',color:Color(0xFFF3EDFF)),
                _PremiumOzellik(icon:Icons.workspace_premium_outlined,title:'Özel rozet',subtitle:'Profilinde fark yarat',color:Color(0xFFEAF5FF)),
                _PremiumOzellik(icon:Icons.query_stats_rounded,title:'Gelişmiş istatistikler',subtitle:'Daha iyi analiz et',color:Color(0xFFE9FAF4)),
              ],
            ),
            const SizedBox(height:16),
            NgelXPremiumCard(
              ...
              child:Row(children:[
                const Icon(Icons.verified_rounded,color:Color(0xFF1687FF),size:40),
                const SizedBox(width:12),
                Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                  const Text('Doğrulanmış Mavi Tik',style:TextStyle(fontSize:16,fontWeight:FontWeight.w900)),
                  const SizedBox(height:3),
                  Text(verified?'Hesabın doğrulanmış.':'Kimlik veya marka doğrulaması için başvur.',style:const TextStyle(color:ngelxPremiumMuted,fontSize:11.5)),
                  const SizedBox(height:3),
                  const Text('Premium’dan ayrı doğrulama sistemidir.',style:TextStyle(color:Color(0xFF1687FF),fontSize:10,fontWeight:FontWeight.w800)),
                ])),
                OutlinedButton(
                  onPressed:verified?null:()=>ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Mavi tik başvuru formu sonraki ödeme/doğrulama paketinde aktif olacak.'))),
                  child:Text(verified?'Doğrulandı':'Başvur'),
                ),
              ]),
            ),
            const SizedBox(height:14),
            NgelXPremiumCard(
              child:Column(children:[
                Row(children:[
                  const Icon(Icons.account_balance_wallet_outlined,color:ngelxPremiumPurple,size:29),
                  const SizedBox(width:10),
                  const Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                    Text('Jeton Cüzdanı',style:TextStyle(fontSize:17,fontWeight:FontWeight.w900)),
                    Text('Hediyeler, öne çıkarma ve topluluk özellikleri',style:TextStyle(color:ngelxPremiumMuted,fontSize:11)),
                  ])),
                  const Icon(Icons.monetization_on_rounded,color:Color(0xFFFFB800)),
                  const SizedBox(width:4),
                  Text(coin.toString()+' Jeton',style:const TextStyle(fontWeight:FontWeight.w900)),
                ]),
                const SizedBox(height:12),
                SizedBox(width:double.infinity,child:FilledButton.icon(
                  style:FilledButton.styleFrom(backgroundColor:ngelxPremiumPurple,padding:const EdgeInsets.symmetric(vertical:13),shape:RoundedRectangleBorder(borderRadius:BorderRadius.circular(16))),
                  onPressed:()=>_magazaHazirDegil(context,'Jeton'),
                  icon:const Icon(Icons.add_card_rounded),
                  label:const Text('Jeton Satın Al',style:TextStyle(fontWeight:FontWeight.w900)),
                )),
              ]),
            ),
            const SizedBox(height:12),
            Row(children:[
              Expanded(child:_JetonPaketi(adet:'100',fiyat:'₺29',onTap:()=>_magazaHazirDegil(context,'100 Jeton'))),
              const SizedBox(width:8),
              Expanded(child:_JetonPaketi(adet:'500',fiyat:'₺99',etiket:'POPÜLER',onTap:()=>_magazaHazirDegil(context,'500 Jeton'))),
              const SizedBox(width:8),
              Expanded(child:_JetonPaketi(adet:'1.200',fiyat:'₺179',onTap:()=>_magazaHazirDegil(context,'1.200 Jeton'))),
            ]),
            const SizedBox(height:18),
            const Text('Jetonlarla Neler Yapabilirsin?',style:TextStyle(fontSize:18,fontWeight:FontWeight.w900)),
            const SizedBox(height:9),
            const NgelXPremiumCard(child:Column(children:[
              ListTile(contentPadding:EdgeInsets.zero,leading:Icon(Icons.card_giftcard_rounded,color:Color(0xFFE63D79)),title:Text('Canlı yayında hediye gönder',style:TextStyle(fontWeight:FontWeight.w800)),subtitle:Text('Favori içerik üreticilerini destekle')),
              Divider(height:1),
              ListTile(contentPadding:EdgeInsets.zero,leading:Icon(Icons.rocket_launch_rounded,color:Color(0xFF1687FF)),title:Text('Gönderini öne çıkar',style:TextStyle(fontWeight:FontWeight.w800)),subtitle:Text('Daha fazla kişiye ulaş')),
              Divider(height:1),
              ListTile(contentPadding:EdgeInsets.zero,leading:Icon(Icons.groups_rounded,color:ngelxPremiumPurple),title:Text('Grubunu veya profilini öne çıkar',style:TextStyle(fontWeight:FontWeight.w800)),subtitle:Text('Topluluğunu büyüt')),
            ])),
            const SizedBox(height:12),
            NgelXPremiumCard(
              onTap:()=>ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Satın alma geçmişi mağaza entegrasyonuyla burada listelenecek.'))),
              child:const Row(children:[
                Icon(Icons.receipt_long_outlined,color:ngelxPremiumMuted),
                SizedBox(width:10),
                Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
                  Text('Satın Alma Geçmişi',style:TextStyle(fontWeight:FontWeight.w900)),
                  Text('Premium ve jeton alımlarını görüntüle',style:TextStyle(color:ngelxPremiumMuted,fontSize:11)),
                ])),
                Icon(Icons.chevron_right_rounded,color:ngelxPremiumMuted),
              ]),
            ),
          ]);
        },
...
class _PremiumOzellik extends StatelessWidget{
  final IconData icon;final String title,subtitle;final Color color;
  const _PremiumOzellik({required this.icon,required this.title,required this.subtitle,required this.color});
  @override Widget build(BuildContext context)=>Container(
    padding:const EdgeInsets.all(12),
    decoration:BoxDecoration(color:color,borderRadius:BorderRadius.circular(19),border:Border.all(color:ngelxPremiumBorder)),
    child:Row(children:[Icon(icon,color:ngelxPremiumPurple,size:25),const SizedBox(width:8),Expanded(child:Column(mainAxisAlignment:MainAxisAlignment.center,crossAxisAlignment:CrossAxisAlignment.start,children:[Text(title,maxLines:2,style:const TextStyle(fontSize:12,fontWeight:FontWeight.w900)),const SizedBox(height:2),Text(subtitle,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(color:ngelxPremiumMuted,fontSize:9.5))]))]),
  );
}

class _JetonPaketi extends StatelessWidget{
  final String adet,fiyat;final String? etiket;final VoidCallback onTap;
  const _JetonPaketi({required this.adet,required this.fiyat,required this.onTap,this.etiket});
  @override Widget build(BuildContext context)=>InkWell(
    onTap:onTap,borderRadius:BorderRadius.circular(18),
    child:Container(
      padding:const EdgeInsets.symmetric(horizontal:7,vertical:11),
      decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(18),border:Border.all(color:etiket==null?ngelxPremiumBorder:ngelxPremiumPurple,width:etiket==null?1:1.4)),
      child:Column(children:[
        if(etiket!=null)Container(padding:const EdgeInsets.symmetric(horizontal:6,vertical:2),decoration:BoxDecoration(color:ngelxPremiumPurple,borderRadius:BorderRadius.circular(8)),child:Text(etiket!,style:const TextStyle(color:Colors.white,fontSize:7.5,fontWeight:FontWeight.w900))),
        if(etiket!=null)const SizedBox(height:4),
        const Icon(Icons.monetization_on_rounded,color:Color(0xFFFFB800),size:27),
        const SizedBox(height:3),
        Text(adet,style:const TextStyle(fontSize:15,fontWeight:FontWeight.w900)),
        const Text('Jeton',style:TextStyle(color:ngelxPremiumMuted,fontSize:10)),
        const SizedBox(height:6),
        Text(fiyat,style:const TextStyle(fontWeight:FontWeight.w900)),
      ]),
    ),
  );
}
        ...
        builder:(_,s){
          ...
          return ListView.separated(
            ...
            itemBuilder:(_,i){
              ...
              final medyaTuru=(v['storyMediaType']??'photo').toString();
              final video=medyaTuru=='video';
              return ListTile(
                leading:ClipRRect(
                  borderRadius:BorderRadius.circular(10),
                  child:video
                ...
                onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>HikayeGosterPage(
                  url:url,mediaType:medyaTuru,kullanici:'Hikâyem',ownerUid:uid,storyId:d.id,createdAt:v['createdAt'],expiresAt:v['expiresAt'],
                ))),
              );
            },
          );
...
class _ProfilPageState extends State<ProfilPage> {
  ...
  int arkadasSayisi = 0;
  int jetonBakiyesi = 0;
  bool premiumAktif = false;
  bool dogrulanmis = false;
  int profilSekme = 0;
  ...
  Future<void> hikayeYukle() async {
    ...
    if(secim==null||!mounted)return;
    final kaynak=secim['source'] as ImageSource;
    final video=secim['video']==true;

    try{
      if(kaynak==ImageSource.camera){
        final kamera=await Permission.camera.request();
        if(!kamera.isGranted){
          if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Kamera izni verilmedi.')));
          return;
        }
        if(video){
          final mikrofon=await Permission.microphone.request();
          if(!mikrofon.isGranted){
            if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Video çekmek için mikrofon izni vermelisin.')));
            return;
          }
        }
      }
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children:[
                      Flexible(child:Text(
                        ad,maxLines:1,overflow:TextOverflow.ellipsis,
                        style: const TextStyle(color:Colors.black,fontSize:23,fontWeight:FontWeight.w900),
                      )),
                      if(dogrulanmis)...[
                        const SizedBox(width:6),
                  ...
                  const SizedBox(height: 22),
                  Row(mainAxisAlignment:MainAxisAlignment.spaceAround,children:[_profilKisayol(Icons.bookmark_border_rounded,t('saved'),tiklama:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const KaydedilenlerPage()))),_profilKisayol(Icons.history_rounded,t('archive'),tiklama:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const HikayeArsiviPage()))),_profilKisayol(Icons.add_circle_outline_rounded,t('stories'),tiklama:hikayeyiAc),_profilKisayol(Icons.event_outlined,t('events'),tiklama:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const ProfilBolumuPage(baslik:'Etkinlikler',aciklama:'Yaklaşan ve katıldığın etkinlikler burada görünecek.',ikon:Icons.event_outlined)))),_profilKisayol(Icons.lock_outline_rounded,t('privacy'),tiklama:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const TercihlerPage(baslik:'Gizlilik')))),_profilKisayol(Icons.settings_outlined,t('settings'),tiklama:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>const AyarlarPage())))]),
                  const SizedBox(height: 20),
                  SizedBox(height:92,child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
                    ...
                    builder:(_,hs){
                      ...
                      return ListView(scrollDirection:Axis.horizontal,children:[
                        ...
                        ...h.take(8).map((d){
                          final v=d.data(),url=(v['mediaUrl']??'').toString(),medyaTuru=(v['storyMediaType']??'photo').toString();
                          final video=medyaTuru=='video';
                          return Padding(
                            padding:const EdgeInsets.only(right:10),
                            child:InkWell(
    ...
    return StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
      ...
      builder:(_,snap){
        ...
        final paylasimlar=(snap.data?.docs??[]).where((d){
          final v=d.data();
          if(v['type']=='story')return false;
          if(profilSekme==1)return (v['type']??'').toString()=='video';
          return true;
        }).toList()
          ..sort((a,b){
            final ap=a.data()['pinned']==true,bp=b.data()['pinned']==true;
  ...
  Widget _etiketlenenGrid(){
    final aranan=kullanici.replaceFirst('@','').trim().toLowerCase();
    if(aranan.isEmpty||aranan=='ngelx')return _profilBosDurum(Icons.alternate_email_rounded,'Etiketlenen içerikleri görmek için kullanıcı adını profilinden ayarla.');
    return StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
      stream:FirebaseFirestore.instance.collection('videos').orderBy('createdAt',descending:true).limit(100).snapshots(),
      builder:(_,snap){
        if(snap.connectionState==ConnectionState.waiting)return const Padding(padding:EdgeInsets.all(38),child:Center(child:CircularProgressIndicator(color:mor)));
        if(snap.hasError)return _profilBosDurum(Icons.cloud_off_outlined,'Etiketlenen içerikler yüklenemedi.');
        final docs=(snap.data?.docs??[]).where((d){
          final v=d.data();
          if(v['type']=='story')return false;
          final tags=(v['tags']??'').toString().toLowerCase();
          final parcali=tags.split(RegExp(r'[\s,;#]+')).map((x)=>x.replaceFirst('@','').trim()).where((x)=>x.isNotEmpty);
          return parcali.contains(aranan);
        }).toList();
        if(docs.isEmpty)return _profilBosDurum(Icons.alternate_email_rounded,'Henüz etiketlendiğin bir paylaşım yok.');
        return _profilBelgeleriGrid(docs,menuAc:false);
      },
    );
  }

  Future<List<DocumentSnapshot<Map<String,dynamic>>>> _begenilenleriGetir()async{
    final uid=aktifKullanici?.uid;
    if(uid==null)return <DocumentSnapshot<Map<String,dynamic>>>[];
    final likes=await FirebaseFirestore.instance.collectionGroup('likes').where('userId',isEqualTo:uid).limit(100).get();
    final refs=<String,DocumentReference<Map<String,dynamic>>>{};
    for(final d in likes.docs){
      final p=d.reference.parent.parent;
      if(p!=null&&p.parent.id=='videos')refs[p.id]=p;
    }
    if(refs.isEmpty)return <DocumentSnapshot<Map<String,dynamic>>>[];
    final docs=await Future.wait(refs.values.map((r)=>r.get()));
    final mevcut=docs.where((d)=>d.exists&&d.data()?['type']!='story').toList();
    mevcut.sort((a,b){
      final at=a.data()?['createdAt'],bt=b.data()?['createdAt'];
      final am=at is Timestamp?at.millisecondsSinceEpoch:0,bm=bt is Timestamp?bt.millisecondsSinceEpoch:0;
      return bm.compareTo(am);
  ...
  Widget _begenilenGrid()=>FutureBuilder<List<DocumentSnapshot<Map<String,dynamic>>>>(
    future:_begenilenleriGetir(),
    builder:(_,snap){
      if(snap.connectionState==ConnectionState.waiting)return const Padding(padding:EdgeInsets.all(38),child:Center(child:CircularProgressIndicator(color:mor)));
      if(snap.hasError)return _profilBosDurum(Icons.cloud_off_outlined,'Beğenilen içerikler yüklenemedi.');
      final docs=snap.data??const <DocumentSnapshot<Map<String,dynamic>>>[];
      if(docs.isEmpty)return _profilBosDurum(Icons.favorite_border_rounded,'Henüz beğendiğin bir paylaşım yok.');
      return _profilBelgeleriGrid(docs,menuAc:false);
    },
  );

  Widget _profilBelgeleriGrid(List<DocumentSnapshot<Map<String,dynamic>>> docs,{required bool menuAc})=>GridView.builder(
    shrinkWrap:true,
    physics:const NeverScrollableScrollPhysics(),
    padding:const EdgeInsets.all(6),
    itemCount:docs.length,
    gridDelegate:const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:3,childAspectRatio:.72,crossAxisSpacing:6,mainAxisSpacing:6),
    itemBuilder:(_,i){
      final belge=docs[i],v=belge.data()??<String,dynamic>{};
      final tur=(v['type']??'video').toString();
      final url=(v['mediaUrl']??v['videoUrl']??'').toString();
      return GestureDetector(
        onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>IcerikBaglantiPage(icerikId:belge.id))),
        onLongPress:menuAc&&belge is QueryDocumentSnapshot<Map<String,dynamic>>?()=>icerikMenusu(belge):null,
        child:Stack(
          fit:StackFit.expand,
          children:[
            MedyaOnizleme(tur:tur,url:url,thumbnailUrl:(v['thumbnailUrl']??'').toString(),yazi:(v['description']??'').toString(),arkaPlan:i.isEven?const Color(0xFF28233F):const Color(0xFF173036)),
            Positioned(left:6,right:6,bottom:6,child:_ProfilEtkilesimRozeti(likeCount:(v['likeCount'] as num?)?.toInt()??0,commentCount:(v['commentCount'] as num?)?.toInt()??0)),
            if(v['pinned']==true&&profilSekme==0)const Positioned(top:6,right:6,child:CircleAvatar(radius:13,backgroundColor:Colors.black54,child:Icon(Icons.push_pin_rounded,color:Colors.white,size:15))),
          ],
        ),
      );
    },
  ...
  Widget _profilBosDurum(IconData ikon,String metin)=>Padding(
    padding:const EdgeInsets.fromLTRB(24,40,24,60),
    child:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[
      Container(width:66,height:66,decoration:const BoxDecoration(color:Color(0xFFF2EEFF),shape:BoxShape.circle),child:Icon(ikon,color:mor,size:32)),
      const SizedBox(height:12),
      Text(metin,textAlign:TextAlign.center,style:const TextStyle(color:Colors.black54,fontSize:14.5,fontWeight:FontWeight.w600)),
    ])),
  );
...
class _ProfilSekme extends StatelessWidget{
  ...
  @override Widget build(BuildContext context)=>InkWell(
    ...
    child:AnimatedContainer(
      ...
      child:Text(yazi,style:TextStyle(color:secili?Colors.black:Colors.black45,fontWeight:secili?FontWeight.w900:FontWeight.w700)),
    ),
  );
}
