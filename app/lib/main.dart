...
class _GrupSohbetPageState extends State<GrupSohbetPage>{
  ...
  Future<void> gonder()async{
    final t=mesaj.text.trim();if(t.isEmpty)return;
    final etiketler=etiketlenenUidler.toList();
    mesaj.clear();
    etiketlenenUidler.clear();
    await payloadGonder({'text':t,'type':'text',if(etiketler.isNotEmpty)'mentions':etiketler},t);
  }
              ...
              : ListView.builder(
                  ...
                  itemBuilder:(_,i){
                    ...
                    return CheckboxListTile(
                      ...
                      secondary:CircleAvatar(backgroundImage:foto.isEmpty?null:NetworkImage(foto)),
  ...
  Future<void> mesajMenusu(QueryDocumentSnapshot<Map<String,dynamic>> d)async{
    ...
    if(fazla=='pin')await d.reference.set({'pinned':v['pinned']!=true,'pinnedAt':FieldValue.serverTimestamp(),'pinnedBy':uid},SetOptions(merge:true));
      ...
      final ok=await showDialog<bool>(context:context,builder:(c)=>AlertDialog(
        backgroundColor:Colors.white,title:const Text('Mesaj silinsin mi?',style:TextStyle(color:Colors.black87,fontWeight:FontWeight.w800)),content:const Text('Mesaj sohbetten kalıcı olarak kaldırılacak.',style:TextStyle(color:Colors.black87)),
      ...
      final ok=await showDialog<bool>(context:context,builder:(c)=>AlertDialog(
        backgroundColor:Colors.white,title:const Text('Mesajı düzenle',style:TextStyle(color:Colors.black87,fontWeight:FontWeight.w800)),content:TextField(controller:x,maxLength:2000,maxLines:5,style:const TextStyle(color:Colors.black87)),
