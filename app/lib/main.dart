...
class _VideoKartiState extends State<VideoKarti> with WidgetsBindingObserver {
  ...
  Future<void> begeniyiDegistir() async {
    ...
    try { if (yeniDurum) {
      ...
      if(mounted)setState(() { begenildi=!yeniDurum; begeniSayisi += yeniDurum ? -1 : 1; });
      if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('Beğeni kaydedilemedi: $e')));
    } finally { begeniIsleniyor=false; }
  }
...
class _MesajPageState extends State<MesajPage> {
  ...
  Future<void> sohbetiAc(String chatId,Widget sayfa) async {
    final ben=uid;
    if(ben!=null)await FirebaseFirestore.instance.collection('chats').doc(chatId).set({'unread_$ben':0},SetOptions(merge:true));
    if(mounted)await Navigator.push(context,MaterialPageRoute(builder:(_)=>sayfa));
  }
        ...
        builder:(_,s){
          ...
          return ListView.builder(itemCount:docs.length,itemBuilder:(_,i){
            ...
            return FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(future:FirebaseFirestore.instance.collection('users').doc(other).get(),builder:(_,u){
              ...
              final aranan='${p['displayName']??''} ${p['username']??''} ${v['lastMessage']??''}'.toLowerCase();
              ...
              return ListTile(
                onTap:()=>sohbetiAc(d.id,SohbetPage(chatId:d.id,digerUid:other,ad:(p['displayName']??p['username']??'NgelX').toString(),foto:(p['photoUrl']??'').toString())),
              ...
              builder: (_, c) {
                ...
                return FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(
                  ...
                  builder: (_, u) {
                    ...
                    return ListTile(
                      leading: CircleAvatar(backgroundImage: foto.isEmpty ? null : NetworkImage(foto)),
    ...
    child: Scaffold(
      ...
      body: FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(
        ...
        builder: (_, me) {
          ...
          return StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
            stream: FirebaseFirestore.instance.collection('chats').where('members', arrayContains: uid).snapshots(),
            builder: (_, s) {
...
class _GrupSohbetPageState extends State<GrupSohbetPage>{
  final mesaj=TextEditingController(),liste=ScrollController();final List<Map<String,String>> mentionOnerileri=[];String? get uid=>FirebaseAuth.instance.currentUser?.uid;
  DocumentReference<Map<String,dynamic>> get chatRef=>FirebaseFirestore.instance.collection('chats').doc(widget.chatId);
  @override void initState(){super.initState();final ben=uid;if(ben!=null)chatRef.set({'unread_$ben':0},SetOptions(merge:true));}
  ...
  Future<void> payloadGonder(Map<String,dynamic> veri,String sonMesaj)async{final ben=uid;if(ben==null||!await mesajGonderilebilir())return;final d=await chatRef.get(),uyeler=List<String>.from(d.data()?['members']??const[]);await chatRef.collection('messages').add({'senderId':ben,'createdAt':FieldValue.serverTimestamp(),...veri});final g=<String,dynamic>{'lastMessage':sonMesaj,'updatedAt':FieldValue.serverTimestamp(),'hiddenFor':FieldValue.arrayRemove(uyeler)};for(final x in uyeler){if(x!=ben)g['unread_$x']=FieldValue.increment(1);}await chatRef.set(g,SetOptions(merge:true));}
  Future<void> gonder()async{final t=mesaj.text.trim();if(t.isEmpty)return;mesaj.clear();await payloadGonder({'text':t,'type':'text'},t);}
  Future<void> medyaGonder(ImageSource kaynak)async{final x=await ImagePicker().pickImage(source:kaynak,imageQuality:82);if(x==null)return;try{final yol='groups/${widget.chatId}/${DateTime.now().millisecondsSinceEpoch}.jpg';await supa.Supabase.instance.client.storage.from('ngelx-media').upload(yol,File(x.path));final url=supa.Supabase.instance.client.storage.from('ngelx-media').getPublicUrl(yol);await payloadGonder({'type':'photo','mediaUrl':url},'📷 Fotoğraf');}catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Fotoğraf gönderilemedi.')));}}
  Future<void> gifGonder()async{const tur=XTypeGroup(label:'GIF',extensions:['gif']);final x=await openFile(acceptedTypeGroups:[tur]);if(x==null)return;try{final yol='groups/${widget.chatId}/${DateTime.now().millisecondsSinceEpoch}.gif';await supa.Supabase.instance.client.storage.from('ngelx-media').uploadBinary(yol,await x.readAsBytes(),fileOptions:const supa.FileOptions(contentType:'image/gif'));final url=supa.Supabase.instance.client.storage.from('ngelx-media').getPublicUrl(yol);await payloadGonder({'type':'gif','mediaUrl':url},'GIF');}catch(e){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('GIF gönderilemedi: $e')));}}
  ...
  Future<void> mentionAra(String deger)async{final parca=deger.split(RegExp(r'\s+')).last;if(!parca.startsWith('@')){if(mentionOnerileri.isNotEmpty&&mounted)setState(()=>mentionOnerileri.clear());return;}final ara=parca.substring(1).toLowerCase(),grup=await chatRef.get(),uyeler=List<String>.from(grup.data()?['members']??const[]),sonuc=<Map<String,String>>[];for(final id in uyeler.take(60)){final d=await FirebaseFirestore.instance.collection('users').doc(id).get(),v=d.data()??{},kullanici=(v['username']??'').toString(),ad=(v['displayName']??kullanici).toString();if(ara.isEmpty||kullanici.toLowerCase().contains(ara)||ad.toLowerCase().contains(ara))sonuc.add({'uid':id,'username':kullanici,'name':ad});if(sonuc.length==5)break;}if(mounted)setState((){mentionOnerileri..clear()..addAll(sonuc);});}
  ...
  Future<void> aramaBaslat(bool goruntulu)async{final ben=uid;if(ben==null)return;final grup=await chatRef.get(),uyeler=List<String>.from(grup.data()?['members']??const[]),grupAdi=(grup.data()?['groupName']??widget.ad).toString(),odaAdi='group_${widget.chatId}_${DateTime.now().millisecondsSinceEpoch}',ref=FirebaseFirestore.instance.collection('calls').doc();await ref.set({'chatId':widget.chatId,'group':true,'title':grupAdi,'roomName':odaAdi,'members':uyeler,'startedBy':ben,'video':goruntulu,'status':'ringing','participants':<String>[ben],'createdAt':FieldValue.serverTimestamp()});for(final uye in uyeler){if(uye!=ben)await uygulamaBildirimiGonder(toUid:uye,fromUid:ben,tur:'call',metin:goruntulu?'$grupAdi grubunda görüntülü arama başlattı':'$grupAdi grubunda sesli arama başlattı',belgeId:ref.id);}if(!mounted)return;await Navigator.push(context,MaterialPageRoute(builder:(_)=>NgelXAramaPage(roomName:odaAdi,baslik:grupAdi,goruntulu:goruntulu,aramaRef:ref)));}

  Widget anketKarti(QueryDocumentSnapshot<Map<String,dynamic>> d,bool benim){final v=d.data(),q=(v['pollQuestion']??'Anket').toString(),opts=List<String>.from(v['pollOptions']??const[]),votes=Map<String,dynamic>.from(v['pollVotes']??{}),toplam=votes.values.fold<int>(0,(a,b)=>a+List.from(b??const[]).length);return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(q,style:TextStyle(color:benim?Colors.white:Colors.black87,fontWeight:FontWeight.w900,fontSize:16)),const SizedBox(height:8),for(int i=0;i<opts.length;i)Padding(padding:const EdgeInsets.only(bottom:6),child:InkWell(onTap:()=>oyVer(d.reference,i),child:Container(padding:const EdgeInsets.symmetric(horizontal:10,vertical:9),decoration:BoxDecoration(color:benim?Colors.white.withValues(alpha:.18):Colors.white,borderRadius:BorderRadius.circular(12),border:Border.all(color:benim?Colors.white38:mor.withValues(alpha:.35))),child:Row(children:[Expanded(child:Text(opts[i],style:TextStyle(color:benim?Colors.white:Colors.black87))),Text('${List.from(votes['$i']??const[]).length}',style:TextStyle(color:benim?Colors.white:mor,fontWeight:FontWeight.bold))])))),Text('$toplam oy',style:TextStyle(fontSize:11,color:benim?Colors.white70:Colors.black45))]);}

  Widget mesajKarti(QueryDocumentSnapshot<Map<String,dynamic>> d){final v=d.data(),ben=v['senderId']==uid,metin=(v['text']??v['message']??v['content']??'').toString(),tur=(v['type']??'text').toString(),media=(v['mediaUrl']??'').toString(),saat=mesajSaati(v['createdAt']),tepkiler=Map<String,dynamic>.from(v['reactions']??{});if(tur=='system')return Center(child:Container(margin:const EdgeInsets.symmetric(vertical:8),padding:const EdgeInsets.symmetric(horizontal:12,vertical:6),decoration:BoxDecoration(color:const Color(0xFFF0F1F4),borderRadius:BorderRadius.circular(14)),child:Text(metin,style:const TextStyle(color:Colors.black54,fontSize:12))));final kutu=GestureDetector(onLongPress:()=>mesajMenusu(d),child:Container(constraints:const BoxConstraints(maxWidth:300),margin:const EdgeInsets.symmetric(vertical:4),padding:EdgeInsets.all((tur=='photo'||tur=='gif')?4:12),decoration:BoxDecoration(color:ben?const Color(0xFF7C3AED):const Color(0xFFF0F1F4),borderRadius:BorderRadius.circular(18)),child:Column(crossAxisAlignment:CrossAxisAlignment.end,children:[if(!ben)FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(future:FirebaseFirestore.instance.collection('users').doc((v['senderId']??'').toString()).get(),builder:(_,u){final p=u.data?.data()??{};return Padding(padding:const EdgeInsets.only(bottom:5),child:Row(mainAxisSize:MainAxisSize.min,children:[CircleAvatar(radius:10,backgroundImage:(p['photoUrl']??'').toString().isEmpty?null:NetworkImage(p['photoUrl'])),const SizedBox(width:5),Flexible(child:Text((p['displayName']??p['username']??'Üye').toString(),style:const TextStyle(color:mor,fontSize:12,fontWeight:FontWeight.w800),overflow:TextOverflow.ellipsis))]));}),if(tur=='photo'||tur=='gif')InkWell(onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>TamEkranMedyaPage(url:media))),child:ClipRRect(borderRadius:BorderRadius.circular(15),child:Image.network(media,width:240,fit:BoxFit.cover)))else if(tur=='poll')anketKarti(d,ben)else Text(metin.isEmpty?'Mesaj içeriği bulunamadı':metin,style:TextStyle(color:ben?Colors.white:Colors.black87,fontSize:16)),if(v['editedAt']!=null)Text('düzenlendi',style:TextStyle(fontSize:9,color:ben?Colors.white60:Colors.black38)),if(tepkiler.isNotEmpty)Wrap(children:tepkiler.values.map((e)=>Text(e.toString())).toList()),if(saat.isNotEmpty)Padding(padding:const EdgeInsets.only(top:4),child:Text(saat,style:TextStyle(fontSize:10,color:ben?Colors.white70:Colors.black45))) ])));return Align(alignment:ben?Alignment.centerRight:Alignment.centerLeft,child:kutu);}

  @override Widget build(BuildContext context)=>Theme(data:ThemeData.light().copyWith(scaffoldBackgroundColor:Colors.white,appBarTheme:const AppBarTheme(backgroundColor:Colors.white,foregroundColor:Colors.black,elevation:0)),child:Scaffold(appBar:AppBar(titleSpacing:0,title:StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(stream:chatRef.snapshots(),builder:(_,s){final v=s.data?.data()??{},ad=(v['groupName']??widget.ad).toString(),foto=(v['groupPhotoUrl']??widget.foto).toString();return Row(children:[CircleAvatar(radius:17,backgroundColor:const Color(0xFFE9DDFF),backgroundImage:foto.isEmpty?null:NetworkImage(foto),child:foto.isEmpty?const Icon(Icons.groups,color:mor,size:20):null),const SizedBox(width:7),Expanded(child:Tooltip(message:ad,child:Text(ad,maxLines:1,overflow:TextOverflow.ellipsis,style:const TextStyle(fontSize:17,fontWeight:FontWeight.w800))))]);}),actions:[IconButton(constraints:const BoxConstraints.tightFor(width:40),tooltip:'Sesli grup araması',onPressed:()=>aramaBaslat(false),icon:const Icon(Icons.call_outlined,color:mor)),IconButton(constraints:const BoxConstraints.tightFor(width:40),tooltip:'Görüntülü grup araması',onPressed:()=>aramaBaslat(true),icon:const Icon(Icons.videocam_outlined,color:mor)),IconButton(constraints:const BoxConstraints.tightFor(width:40),tooltip:'Grup bilgileri',onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>GrupBilgiPage(chatId:widget.chatId))),icon:const Icon(Icons.info_outline,color:mor))]),body:Column(children:[Expanded(child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(stream:chatRef.collection('messages').orderBy('createdAt').snapshots(),builder:(_,s){if(uid!=null&&s.hasData)chatRef.set({'unread_$uid':0},SetOptions(merge:true));if(s.hasData)sonaGit();return ListView(controller:liste,padding:const EdgeInsets.all(12),children:(s.data?.docs??[]).map(mesajKarti).toList());})),if(mentionOnerileri.isNotEmpty)Container(color:Colors.white,child:Column(mainAxisSize:MainAxisSize.min,children:mentionOnerileri.map((u)=>ListTile(dense:true,leading:const CircleAvatar(radius:15,child:Icon(Icons.person,size:17)),title:Text(u['name']??'Üye'),subtitle:Text('@${u['username']??''}'),onTap:()=>mentionEkle(u['username']??''))).toList())),SafeArea(top:false,child:Padding(padding:const EdgeInsets.fromLTRB(8,7,8,8),child:Row(children:[IconButton(tooltip:'Dosya ve medya ekle',onPressed:ekMenusu,icon:const Icon(Icons.add_circle_outline,color:mor)),Expanded(child:TextField(controller:mesaj,onChanged:mentionAra,onSubmitted:(_)=>gonder(),maxLength:2000,buildCounter:(_, {required currentLength,required isFocused,maxLength})=>null,decoration:const InputDecoration(hintText:'Gruba mesaj yaz...'))),IconButton(tooltip:'Emoji seç',onPressed:emojiSec,icon:const Icon(Icons.emoji_emotions_outlined,color:mor)),IconButton(tooltip:'Gönder',onPressed:gonder,icon:const Icon(Icons.send_rounded,color:mor))])))])));
}
...
class _SohbetPageState extends State<SohbetPage> {
  ...
  Future<void> mesajMenusu(QueryDocumentSnapshot<Map<String,dynamic>> d)async{
    ...
    final sec=await showModalBottomSheet<String>(context:context,backgroundColor:Colors.white,showDragHandle:true,builder:(c)=>SafeArea(child:Column(mainAxisSize:MainAxisSize.min,children:[
      Wrap(spacing:10,children:['❤️','👍','😂','😮','😢','😡'].map((e)=>TextButton(onPressed:()=>Navigator.pop(c,'reaction:$e'),child:Text(e,style:const TextStyle(fontSize:24)))).toList()),
      ListTile(leading:const Icon(Icons.reply),title:const Text('Yanıtla'),onTap:()=>Navigator.pop(c,'reply')),
      if(metin.isNotEmpty)ListTile(leading:const Icon(Icons.copy),title:const Text('Kopyala'),onTap:()=>Navigator.pop(c,'copy')),
      if(benim&&metin.isNotEmpty)ListTile(leading:const Icon(Icons.edit),title:const Text('Düzenle'),onTap:()=>Navigator.pop(c,'edit')),
    ...
    if(sec==null)return;
    if(sec.startsWith('reaction:'))await d.reference.set({'reactions.${uid!}':sec.substring(9)},SetOptions(merge:true));
    else if(sec=='copy')await Clipboard.setData(ClipboardData(text:metin));
    else if(sec=='reply'){mesaj.text='↪ $metin\n';mesaj.selection=TextSelection.collapsed(offset:mesaj.text.length);}
    else if(sec=='pin')await d.reference.set({'pinned':v['pinned']!=true,'pinnedAt':FieldValue.serverTimestamp(),'pinnedBy':uid},SetOptions(merge:true));
    else if(sec=='delete'){final ok=await showDialog<bool>(context:context,builder:(c)=>AlertDialog(backgroundColor:Colors.white,title:const Text('Mesaj silinsin mi?'),content:const Text('Mesaj sohbetten kalıcı olarak kaldırılacak.'),actions:[TextButton(onPressed:()=>Navigator.pop(c,false),child:const Text('Vazgeç')),FilledButton(style:FilledButton.styleFrom(backgroundColor:Colors.red),onPressed:()=>Navigator.pop(c,true),child:const Text('Sil'))]))??false;if(ok)await d.reference.delete();}
    else if(sec=='report'){if(mounted)await sikayetEt(context,hedefTuru:'mesaj',hedefId:'${widget.chatId}/${d.id}',hedefUid:widget.digerUid);}
    else if(sec=='edit'){final c=TextEditingController(text:metin);final ok=await showDialog<bool>(context:context,builder:(x)=>AlertDialog(backgroundColor:Colors.white,title:const Text('Mesajı düzenle'),content:TextField(controller:c,maxLength:2000,maxLines:5),actions:[TextButton(onPressed:()=>Navigator.pop(x,false),child:const Text('Vazgeç')),FilledButton(onPressed:()=>Navigator.pop(x,true),child:const Text('Kaydet'))]))??false;if(ok&&c.text.trim().isNotEmpty)await d.reference.update({'text':c.text.trim(),'editedAt':FieldValue.serverTimestamp()});c.dispose();}
  }

  Widget ozelMesajKarti(QueryDocumentSnapshot<Map<String,dynamic>> d){final v=d.data(),ben=v['senderId']==uid,photo=v['type']=='photo',shared=v['type']=='shared_content',metin=(v['text']??v['message']??v['content']??'').toString().trim(),saat=mesajSaati(v['createdAt']),tepkiler=Map<String,dynamic>.from(v['reactions']??{});return Align(alignment:ben?Alignment.centerRight:Alignment.centerLeft,child:GestureDetector(onLongPress:()=>mesajMenusu(d),child:Container(constraints:const BoxConstraints(maxWidth:280),margin:const EdgeInsets.all(4),padding:EdgeInsets.all(photo?4:12),decoration:BoxDecoration(color:ben?const Color(0xFF1687FF):const Color(0xFFF0F1F4),borderRadius:BorderRadius.circular(18)),child:Column(crossAxisAlignment:CrossAxisAlignment.end,mainAxisSize:MainAxisSize.min,children:[if(photo)InkWell(onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>TamEkranMedyaPage(url:(v['mediaUrl']??'').toString()))),child:ClipRRect(borderRadius:BorderRadius.circular(15),child:Image.network((v['mediaUrl']??'').toString(),width:220,fit:BoxFit.cover)))else if(shared)InkWell(onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>IcerikBaglantiPage(icerikId:(v['contentId']??'').toString()))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[Icon(Icons.play_circle_fill_rounded,color:ben?Colors.white:mavi),const SizedBox(width:7),Text('NgelX paylaşımı',style:TextStyle(color:ben?Colors.white:Colors.black87,fontWeight:FontWeight.w900))]),const SizedBox(height:8),Text(metin,maxLines:4,overflow:TextOverflow.ellipsis,style:TextStyle(color:ben?Colors.white70:Colors.black54))]))else Text(metin.isEmpty?'Mesaj içeriği bulunamadı':metin,softWrap:true,style:TextStyle(color:ben?Colors.white:Colors.black87,fontSize:16,height:1.3)),if(v['editedAt']!=null)Text('düzenlendi',style:TextStyle(fontSize:9,color:ben?Colors.white60:Colors.black38)),if(tepkiler.isNotEmpty)Wrap(children:tepkiler.values.map((e)=>Text(e.toString())).toList()),if(saat.isNotEmpty)Padding(padding:const EdgeInsets.only(top:4),child:Text(saat,style:TextStyle(fontSize:10,color:ben?Colors.white70:Colors.black45))) ])))));}
  ...
  Widget build(BuildContext context)=>Theme(data:ThemeData.light().copyWith(scaffoldBackgroundColor:Colors.white,appBarTheme:const AppBarTheme(backgroundColor:Colors.white,foregroundColor:Colors.black,elevation:0)),child:Scaffold(
    ...
    body:StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('chats').doc(widget.chatId).snapshots(),builder:(_,tema){final ham=tema.data?.data()?['theme_$uid'];final arkaPlan=ham is int?Color(ham):Colors.white;return ColoredBox(color:arkaPlan,child:Column(children:[
      Expanded(child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
...
class SohbetBilgiPage extends StatelessWidget{
  ...
  Future<void> ozellestir(BuildContext context)async{final me=FirebaseAuth.instance.currentUser?.uid;if(me==null)return;final renk=await showModalBottomSheet<int>(context:context,backgroundColor:Colors.white,showDragHandle:true,builder:(c)=>SafeArea(child:Column(mainAxisSize:MainAxisSize.min,children:[const ListTile(title:Text('Sohbet arka planı',style:TextStyle(fontWeight:FontWeight.w900))),Wrap(spacing:16,runSpacing:16,children:[Colors.white,const Color(0xFFFFF4F7),const Color(0xFFF4F0FF),const Color(0xFFEFF8FF),const Color(0xFFF1FFF5)].map((x)=>InkWell(onTap:()=>Navigator.pop(c,x.toARGB32()),child:CircleAvatar(radius:25,backgroundColor:x,child:const Icon(Icons.check,color:Colors.black26)))).toList()),const SizedBox(height:22)])));if(renk!=null)await FirebaseFirestore.instance.collection('chats').doc(chatId).set({'theme_$me':renk},SetOptions(merge:true));}
  ...
  Widget _satir(IconData i,String t,VoidCallback f,{Color? renk})=>ListTile(onTap:f,leading:Icon(i,color:renk),title:Text(t,style:TextStyle(color:renk)),trailing:const Icon(Icons.chevron_right));
}
          ...
          return ListView(
            ...
            children: [
              ...
              Row(mainAxisAlignment:MainAxisAlignment.spaceEvenly,children:[
                ...
                _profilSayac(context,'•','Etkileşim',()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>EtkilesimOzetiPage(uid:uid)))),
                  ...
                  builder: (_, p) {
                    ...
                    return GridView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return GestureDetector(onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>IcerikBaglantiPage(icerikId:docs[i].id))),child:MedyaOnizleme(tur: tur, url: url, yazi: (x['description'] ?? '').toString(), arkaPlan: const Color(0xFFF0F1F4)));
                      },
                    );
                  },
                )]),
...
class _ProfilAramaPageState extends State<ProfilAramaPage>{
  ...
  @override Widget build(BuildContext context)=>Theme(
    ...
    child:Scaffold(
      ...
      body:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
        stream:FirebaseFirestore.instance.collection('videos').where('ownerId',isEqualTo:widget.uid).snapshots(),
        builder:(_,s){
