...
class _MesajPageState extends State<MesajPage> {
      ...
      Expanded(child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
        ...
        builder:(_,s){
          final docs=(s.data?.docs??[]).where((d){final v=d.data();if(List<String>.from(v['hiddenFor']??const[]).contains(ben))return false;final grup=v['isGroup']==true||List<String>.from(v['members']??const[]).length>2;final unread=(v['unread_$ben']??0) as int;if(filtre=='Okunmamış'&&unread==0)return false;if(filtre=='Gruplar'&&!grup)return false;if(filtre=='Arkadaşlar'&&grup)return false;final son=(v['lastMessage']??'').toString().toLowerCase();return sohbetSorgu.isEmpty||son.contains(sohbetSorgu);}).toList()
            ..sort((a,b)=>_tarihDegeri(b.data()['updatedAt']).compareTo(_tarihDegeri(a.data()['updatedAt'])));
          ...
          return ListView.builder(itemCount:docs.length,itemBuilder:(_,i){
            ...
            return FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(future:FirebaseFirestore.instance.collection('users').doc(other).get(),builder:(_,u){
              ...
              return ListTile(
                ...
                trailing:Column(mainAxisAlignment:MainAxisAlignment.center,crossAxisAlignment:CrossAxisAlignment.end,children:[Text(sohbetZamani(v['updatedAt']),style:TextStyle(color:unread>0?mor:Colors.black45,fontSize:11,fontWeight:unread>0?FontWeight.w800:FontWeight.w500)),const SizedBox(height:4),Wrap(crossAxisAlignment:WrapCrossAlignment.center,spacing:5,children:[if(sessiz)const Icon(Icons.volume_off,size:17,color:Colors.black38),if(unread>0)Badge(label:Text('$unread'))])]),
              );
            });
          });
        },
  ...
  Widget _sohbetListeSonu(dynamic tarih,int okunmamis)=>Column(mainAxisAlignment:MainAxisAlignment.center,crossAxisAlignment:CrossAxisAlignment.end,children:[Text(sohbetZamani(tarih),style:TextStyle(color:okunmamis>0?mor:Colors.black45,fontSize:11,fontWeight:okunmamis>0?FontWeight.w800:FontWeight.w500)),const SizedBox(height:4),if(okunmamis>0)Badge(label:Text('$okunmamis'))]);
}
...
class _GrupSohbetPageState extends State<GrupSohbetPage>{final mesaj=TextEditingController();String? get uid=>FirebaseAuth.instance.currentUser?.uid;@override void dispose(){mesaj.dispose();super.dispose();}
  ...
  @override Widget build(BuildContext context)=>Theme(data:ThemeData.light().copyWith(scaffoldBackgroundColor:Colors.white,appBarTheme:const AppBarTheme(backgroundColor:Colors.white,foregroundColor:Colors.black,elevation:0)),child:Scaffold(appBar:AppBar(title:Row(children:[CircleAvatar(backgroundColor:const Color(0xFFE9DDFF),backgroundImage:widget.foto.isEmpty?null:NetworkImage(widget.foto),child:widget.foto.isEmpty?const Icon(Icons.groups,color:mor):null),const SizedBox(width:10),Expanded(child:Text(widget.ad,style:const TextStyle(fontWeight:FontWeight.w800)))]),actions:[IconButton(onPressed:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>GrupBilgiPage(chatId:widget.chatId))),icon:const Icon(Icons.info_outline,color:mor))]),body:Column(children:[Expanded(child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(stream:FirebaseFirestore.instance.collection('chats').doc(widget.chatId).collection('messages').orderBy('createdAt',descending:true).snapshots(),builder:(_,s)=>ListView(padding:const EdgeInsets.fromLTRB(12,12,12,20),children:(s.data?.docs??[]).map((d){final v=d.data(),ben=v['senderId']==uid;return mesajBalonu(v,ben,benimRenk:const Color(0xFF7C3AED));}).toList()))),SafeArea(top:false,child:Padding(padding:const EdgeInsets.fromLTRB(8,7,8,10),child:Row(children:[IconButton(onPressed:(){},icon:const Icon(Icons.add_circle_outline,color:mor)),Expanded(child:TextField(controller:mesaj,onSubmitted:(_)=>gonder(),decoration:const InputDecoration(hintText:'Gruba mesaj yaz...'))),IconButton(onPressed:gonder,icon:const Icon(Icons.send_rounded,color:mor))])))])));
}
  ...
  Widget build(BuildContext context)=>Theme(data:ThemeData.light().copyWith(scaffoldBackgroundColor:Colors.white,appBarTheme:const AppBarTheme(backgroundColor:Colors.white,foregroundColor:Colors.black,elevation:0)),child:Scaffold(
    ...
    body:Column(children:[
      Expanded(child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
        ...
        builder:(_,s)=>ListView(padding:const EdgeInsets.fromLTRB(12,12,12,20),children:(s.data?.docs??[]).map((d){final v=d.data(),ben=v['senderId']==uid;return mesajBalonu(v,ben);}).toList()),
      )),
...
DateTime _tarihDegeri(dynamic t)=>t is Timestamp?t.toDate():t is DateTime?t:DateTime.fromMillisecondsSinceEpoch(0);
String _iki(int n)=>n.toString().padLeft(2,'0');
String mesajSaati(dynamic t){final d=_tarihDegeri(t);if(d.millisecondsSinceEpoch==0)return 'Gönderiliyor…';return '${_iki(d.hour)}:${_iki(d.minute)}';}
String sohbetZamani(dynamic t){final d=_tarihDegeri(t);if(d.millisecondsSinceEpoch==0)return '';final simdi=DateTime.now(),bugun=DateTime(simdi.year,simdi.month,simdi.day),gun=DateTime(d.year,d.month,d.day);if(gun==bugun)return mesajSaati(t);if(gun==bugun.subtract(const Duration(days:1)))return 'Dün';return '${_iki(d.day)}.${_iki(d.month)}.${d.year}';}

Widget mesajBalonu(Map<String,dynamic> veri,bool benim,{Color benimRenk=const Color(0xFF1687FF)}){
  final fotograf=veri['type']=='photo'||(veri['mediaUrl']??'').toString().isNotEmpty;
  final metin=(veri['text']??veri['message']??veri['content']??'').toString().trim();
  final yaziRengi=benim?Colors.white:Colors.black87;
  return Align(alignment:benim?Alignment.centerRight:Alignment.centerLeft,child:Container(
    constraints:const BoxConstraints(maxWidth:285),margin:const EdgeInsets.symmetric(vertical:4,horizontal:2),padding:EdgeInsets.all(fotograf?5:11),
    decoration:BoxDecoration(color:benim?benimRenk:const Color(0xFFF0F1F4),borderRadius:BorderRadius.circular(18),border:fotograf?Border.all(color:benim?benimRenk:const Color(0xFFDDE1E8)):null),
    child:Column(mainAxisSize:MainAxisSize.min,crossAxisAlignment:CrossAxisAlignment.end,children:[
      if(fotograf)ClipRRect(borderRadius:BorderRadius.circular(14),child:Image.network((veri['mediaUrl']??'').toString(),width:235,height:235,fit:BoxFit.cover,errorBuilder:(_,__,___)=>SizedBox(width:235,height:120,child:Center(child:Icon(Icons.broken_image_outlined,color:yaziRengi))))),
      if(metin.isNotEmpty)Padding(padding:EdgeInsets.fromLTRB(fotograf?7:1,fotograf?7:0,fotograf?7:1,2),child:Align(alignment:Alignment.centerLeft,child:Text(metin,softWrap:true,style:TextStyle(color:yaziRengi,fontSize:16,height:1.3)))),
      if(!fotograf&&metin.isEmpty)Text('Mesaj içeriği bulunamadı',style:TextStyle(color:yaziRengi,fontSize:16)),
      Padding(padding:EdgeInsets.fromLTRB(fotograf?7:1,2,fotograf?5:1,0),child:Text(mesajSaati(veri['createdAt']),style:TextStyle(color:benim?Colors.white70:Colors.black45,fontSize:10,fontWeight:FontWeight.w600))),
    ]),
  ));
}
...
class ArkadaslarPage extends StatelessWidget {const ArkadaslarPage({super.key});@override Widget build(BuildContext context){final uid=FirebaseAuth.instance.currentUser?.uid;return Scaffold(appBar:AppBar(title:const Text('Arkadaşlar')),body:FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(future:uid==null?null:FirebaseFirestore.instance.collection('users').doc(uid).get(),builder:(_,s){final ids=List<String>.from(s.data?.data()?['friends']??[]);if(ids.isEmpty)return const Center(child:Text('Henüz arkadaşın yok.'));return ListView(children:ids.map((id)=>FutureBuilder<DocumentSnapshot<Map<String,dynamic>>>(future:FirebaseFirestore.instance.collection('users').doc(id).get(),builder:(_,u){final v=u.data?.data()??{},foto=(v['photoUrl']??'').toString();return ListTile(onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>KullaniciProfilPage(uid:id))),onLongPress:()async{final ad=(v['displayName']??v['username']??'Bu kişi').toString();final ok=await showDialog<bool>(context:context,builder:(c)=>AlertDialog(title:const Text('Arkadaşlıktan çıkarılsın mı?'),content:Text('$ad arkadaşlıktan çıkarılsın mı?'),actions:[TextButton(onPressed:()=>Navigator.pop(c,false),child:const Text('Vazgeç')),TextButton(onPressed:()=>Navigator.pop(c,true),child:const Text('ARKADAŞLIKTAN ÇIKAR',style:TextStyle(color:Colors.red,fontWeight:FontWeight.bold)))]));if(ok==true&&uid!=null){try{final batch=FirebaseFirestore.instance.batch();batch.set(FirebaseFirestore.instance.collection('users').doc(uid),{'friends':FieldValue.arrayRemove([id])},SetOptions(merge:true));batch.set(FirebaseFirestore.instance.collection('users').doc(id),{'friends':FieldValue.arrayRemove([uid])},SetOptions(merge:true));await batch.commit();if(context.mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text('$ad arkadaşlarından çıkarıldı.')));}catch(_){if(context.mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Arkadaşlıktan çıkarılamadı. Tekrar dene.')));}}},leading:CircleAvatar(backgroundImage:foto.isEmpty?null:NetworkImage(foto)),title:Text((v['displayName']??v['username']??'NgelX').toString()),subtitle:Text('@${v['username']??'ngelx'}'),trailing:const Icon(Icons.chevron_right));})).toList());}));}}
                  ...
                  return GridView.builder(
                    ...
                    itemBuilder: (_, i) => FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                      ...
                      builder: (_, v) {
                        ...
                        return GestureDetector(
                          onTap: veri.isEmpty ? null : () {
                            final item=<String,String>{'id':(docs[i].data()['contentId']??docs[i].id).toString(),'type':tur,'videoUrl':(veri['videoUrl']??'').toString(),'mediaUrl':url,'audioUrl':(veri['audioUrl']??'').toString(),'description':(veri['description']??'').toString(),'username':(veri['username']??'ngelx').toString(),'ownerId':(veri['ownerId']??'').toString(),'allowDownload':(veri['allowDownload']??true).toString()};
                            Navigator.push(context,MaterialPageRoute(builder:(_)=>Scaffold(backgroundColor:Colors.black,appBar:AppBar(backgroundColor:Colors.black,foregroundColor:Colors.white),body:SafeArea(child:tur=='video'?VideoKarti(adres:item['videoUrl']!,videoId:item['id']!,kullaniciAdi:item['username']!,ownerId:item['ownerId']!,indirilebilir:item['allowDownload']!='false',aktif:true):GorselYaziKarti(veri:item,aktif:true)))));
                          },
