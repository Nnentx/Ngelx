  ...
  @override Widget build(BuildContext context){
    ...
    return Theme(data:ThemeData.light().copyWith(scaffoldBackgroundColor:Colors.white),child:ColoredBox(color:Colors.white,child:SafeArea(child:Column(children:[
      ...
      Expanded(child:StreamBuilder<QuerySnapshot<Map<String,dynamic>>>(
        stream:ben==null?null:FirebaseFirestore.instance.collection('chats').where('members',arrayContains:ben).limit(200).snapshots(),
        builder:(_,s){
...
class _GrupOlusturPageState extends State<GrupOlusturPage>{
  ...
  Future<void> olustur()async{
    ...
    try{
      ...
      unawaited(ref.set({'isGroup':true,'groupName':grupAdi,'groupPhotoUrl':fotoUrl,'members':[u.uid,...secilen],'admins':[u.uid],'moderators':<String>[],'createdBy':u.uid,'createdAt':FieldValue.serverTimestamp(),'updatedAt':FieldValue.serverTimestamp(),'lastMessage':'Grup oluşturuldu','hiddenFor':<String>[],'maxMembers':60,'onlyAdminsCanEdit':true}).timeout(const Duration(seconds:12)).catchError((_){
        if(mounted)ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Grup sunucuya gönderilmek üzere kaydedildi. Bağlantı gelince tamamlanacak.')));
      }));
      if(mounted){
...
class _SohbetPageState extends State<SohbetPage> {
  ...
  Widget build(BuildContext context)=>Theme(data:ThemeData.light().copyWith(scaffoldBackgroundColor:Colors.white,appBarTheme:const AppBarTheme(backgroundColor:Colors.white,foregroundColor:Colors.black,elevation:0)),child:Scaffold(
    appBar:AppBar(
      ...
      title:StreamBuilder<DocumentSnapshot<Map<String,dynamic>>>(stream:_chatAkisi,builder:(_,s){final ham=s.data?.data()?['nicknames'],nicks=ham is Map?Map<String,dynamic>.from(ham):<String,dynamic>{},takma=(uid==null?'':(nicks[uid]??'').toString()).trim(),gorunenAd=takma.isNotEmpty?takma:widget.ad;return InkWell(onTap:()=>Navigator.push(context,MaterialPageRoute(builder:(_)=>KullaniciProfilPage(uid:widget.digerUid))),child:Row(children:[CircleAvatar(radius:18,backgroundImage:widget.foto.isEmpty?null:CachedNetworkImageProvider(widget.foto)),const SizedBox(width:9),Expanded(child:Text(gorunenAd))]));}),
      actions:[
        IconButton(
          tooltip:'Sesli arama',
